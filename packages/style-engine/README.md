...
of throwing. `config.markers` (up to `MAX_MARKERS`, see `@topolyne/config-schema`) are rendered
as their own GeoJSON source and pin/label layers automatically — nothing extra to call for
those. Setting `config.terrain.satellite` swaps the vector-tile land styling for real aerial
imagery from `provider.satelliteTiles` as the base layer; roads, borders, labels, and buildings
still render on top.
...
OpenFreeMap for vector tiles, MapTiler terrain-RGB for elevation and MapTiler satellite tiles for
aerial imagery (both only when `NEXT_PUBLIC_MAPTILER_API_KEY` is set in the environment —
otherwise `demTiles`/`satelliteTiles` are left `undefined` and those layers are skipped),
Nominatim for geocoding, and OpenRouteService for directions. Note the `NEXT_PUBLIC_` prefix:
`buildMapStyle()` runs client-side wherever `defaultProvider` is used, so the key needs to reach
the browser bundle — this is the same trust model MapTiler (and Mapbox) already design their
keys for, restricting abuse by domain rather than by secrecy. `withProviderOverrides`
...
this from a server route, not directly from the browser (mirrored by the hosted app's
`/api/geocode`).
...
never from the browser (mirrored by the hosted app's `/api/directions`, which is what
